...
func (h *Handlers) CareerCreate(w http.ResponseWriter, r *http.Request) {
	if !h.hasCoreServices() {
		writeError(w, http.StatusInternalServerError, errServicesNotReady)
		return
	}
	var payload models.CareerCreateRequest
	if err := decodeJSONBody(r, &payload); err != nil {
		writeError(w, http.StatusBadRequest, errInvalidJSONPayload)
		return
	}
	result, err := h.services.Career.Create(r.Context(), payload)
	if err != nil {
